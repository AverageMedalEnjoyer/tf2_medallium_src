			...
			if ( !AreEnemyTeams( g_PR->GetTeam( playerIndex ), localteam ) && !( localteam == TEAM_UNASSIGNED ) )
			{
				...
				if ( g_PR->IsConnected( playerIndex ) )
				{
					...
					if ( iClass >= TF_FIRST_NORMAL_CLASS && iClass < TF_LAST_NORMAL_CLASS )
					{
						...
						{
							pKeyValues->SetInt( "class", tf_scoreboard_alt_class_icons.GetBool() ? m_iImageClassAlt[iClass + ( TF_LAST_NORMAL_CLASS - TF_FIRST_NORMAL_CLASS )] : m_iImageClass[iClass + ( TF_LAST_NORMAL_CLASS - TF_FIRST_NORMAL_CLASS )] ); // Skip the living class icons to select the darker dead icons.
						}
					}
					else
					{
