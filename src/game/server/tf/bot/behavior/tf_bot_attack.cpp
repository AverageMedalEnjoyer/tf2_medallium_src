...
ActionResult< CTFBot >	CTFBotAttack::Update( CTFBot *me, float interval )
{
		 ...
		 !me->IsLineOfFireClear( threat->GetEntity()->EyePosition() ) )
	{
		if ( threat->IsVisibleRecently() )
		{
			if ( isUsingCloseRangeWeapon && !( TFGameRules()->IsMannVsMachineMode() && me->GetTeamNumber() == TF_TEAM_PVE_INVADERS ) )	// all bots in MvM use the default route
			{
		...
		else
		{
			...
			if ( m_repathTimer.IsElapsed() )
			{
				...
				{
					CTFBotPathCost cost( me, DEFAULT_ROUTE );
					float maxPathLength = TFGameRules()->IsMannVsMachineMode() && me->GetTeamNumber() == TF_TEAM_PVE_INVADERS ? TFBOT_MVM_MAX_PATH_LENGTH : 0.0f;
